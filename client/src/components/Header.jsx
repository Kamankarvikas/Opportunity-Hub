  ...
  return (
    <div className={`flex w-full md:w-1/3 items-center ${styles}`}>
      ...
      <input
        ...
        type='text'
        className='w-full p-2 text-base bg-transparent outline-none md:w-64'
        placeholder={placeholder}
      />

      <AiOutlineCloseCircle
        className='hidden text-xl text-gray-600 cursor-pointer md:flex'
        onClick={clearInput}
      />
    </div>
    ...
    <div className='bg-[#f7fdfd]'>
      ...
      >
        <div className='z-10 w-full'>
          <div className='mb-8'>
            <p className='text-4xl font-bold text-slate-700'>{title}</p>
          </div>

          <div className='w-full flex items-center justify-around bg-white px-2 md:px-5 py-2.5 md:py-6 shadow-2xl rounded-full'>
            <SearchInput
              placeholder='Job Title or Keywords'
              icon={<AiOutlineSearch className='text-xl text-gray-600' />}
              value={searchQuery}
              setValue={setSearchQuery}
            />
            <SearchInput
              placeholder='Add Country or City'
              icon={<CiLocationOn className='text-xl text-gray-600' />}
              value={location}
          ...
          {type && (
            <div className='flex flex-wrap w-full gap-3 py-10 lg:1/2 md:gap-6 md:py-14'>
